...
<div id="containerdash">
	<div id="messagearea">
			...
			<tr>
				<td>
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="projfiles" >
						...
						<c:forEach items="${contactUsList}" var="contactUs" varStatus="counter">
							<c:if test="${not contactUs.company.generic}">
							<c:url var="url" value="admincontactus.do" >
								<c:param name="action" value="adminShowContactUs"/>
								<c:param name="contactusid" value="${contactUs.id}"/>
							</c:url>
							<tr class="bground">						
								<td width="100%" class="itemthumb">
									<span>${contactUs.company.name}</span>
									<div align="right">
									<a class="editdel" href="<c:out value="${url}"/>">edit</a>
										<span class="projdetails">|</span>
									<a class="editdel" href="admincontactus.do?action=adminDeleteContactUs&id=<c:out value="${contactUs.id}"/>" onclick="return confirm('Are you sure you want to delete\n the #<c:out value='${counter.count}' /> page?')">delete</a>
									</div>
								</td>
							</tr>
							</c:if>
		...
		<form method="post" action="admincontactus.do" onsubmit="return validate(this);">
			<input type="hidden" name="action" value="adminAddContactUs">
			<table border="0" cellpadding="0" cellspacing="0" width="99%" class="projfiles" >
				<tr>
					<th><img src="images/admin/add.png" width="16" height="16" border="0"/>Add New Contact Us Page</th>
				</tr>
				<tr class="bground">
					<td class="itemthumb">
						<%@ include file="../includes/error.jsp"%>
							...
							<tr>
								...
								<td>
									...
									<select name="company_id" class="item">
										...
										<c:if test="${not company.generic and company.contactUs eq null}">
